...
/// Helper: poll a log scanner until we collect expected_count records, or timeout.
inline std::vector<fluss::ScanRecord> PollRecords(fluss::LogScanner& scanner,
    ...
    // Instead, we'll poll and collect data we need

    // Actually, ScanRecord borrows from ScanRecords, which borrows from the inner Rust data.
    // We can't return ScanRecords in a vector since they're move-only. Instead let's
    // accumulate all ScanRecords objects and return a flattened view.
    // For simplicity, we'll return a vector of extracted values directly.

    // Wait - actually the caller needs the ScanRecord to read fields. The problem is
    // ScanRecord has a RowView that borrows from ScanRecords. So we need to keep
    // the ScanRecords alive while using ScanRecord.

    // For the tests, it's better to collect ScanRecords objects and then use them.
    // But since we need a flat vector of ScanRecord, we need a different approach.

    // Let's just return here with a collected count and let the caller handle the polling.
    // Actually, the simplest pattern is to poll in a loop and process inline.
    // Let me return nothing and let the test do the polling.

    // Hmm, actually we CAN return ScanRecords if we keep them in a vector.
    // The issue is that ScanRecord has a RowView borrowing from ScanRecords.
    // But if we never move the ScanRecords after creating ScanRecords, the borrow is valid.

    // This is getting complicated. Let me just provide a simpler helper.
    std::vector<fluss::ScanRecord> records;
    return records;  // placeholder - tests will poll inline
}

/// Helper: get a fresh connection from the test environment.
inline fluss::Connection GetConnection() {
    auto* env = FlussTestEnvironment::Instance();
    fluss::Configuration config;
    config.bootstrap_servers = env->GetBootstrapServers();
    fluss::Connection conn;
    auto result = fluss::Connection::Create(config, conn);
    if (!result.Ok()) {
        throw std::runtime_error("Failed to create connection: " + result.error_message);
    }
    return conn;
}

/// Helper: get a table from a new connection.
inline fluss::Table GetTable(const fluss::TablePath& path) {
    auto conn = GetConnection();
    fluss::Table table;
    auto result = conn.GetTable(path, table);
    if (!result.Ok()) {
        throw std::runtime_error("Failed to get table: " + result.error_message);
    }
    return table;
}
