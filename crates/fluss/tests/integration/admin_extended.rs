...
mod admin_extended_test {
    ...
    fn assert_expected_api_error(error: fluss::error::Error, allowed: &[FlussError]) {
        match error.api_error() {
            ...
            other => {
                panic!("Expected one of {allowed:?} but got {other:?} (full error: {error:?})")
            }
        }
    }
    ...
    async fn test_alter_database() {
        ...
        let config_change = AlterConfig::new(
            "custom.key",
    ...
    async fn test_alter_cluster_configs() {
        ...
        let Some(existing) = described.iter().find(|c| c.config_value.is_some()) else {
            // No config with a value to round-trip; nothing to assert.
            return;
        };

        let alter = AlterConfig::new(
            existing.config_key.clone(),
            existing.config_value.clone(),
            AlterConfigOpType::Set,
        );
    ...
    async fn test_get_table_stats() {
        ...
        let buckets_req = vec![BucketStatsRequest::new(None, 0)];
        ...
        match admin.get_table_stats(table_id, buckets_req, vec![]).await {
            Ok(response) => {
                for bucket in &response.buckets {
                    // Per-bucket entries echo the requested bucket id.
                    assert_eq!(
                        bucket.bucket_id, 0,
                        "unexpected bucket id in stats response"
                    );
                }
            }
            Err(fluss::error::Error::UnsupportedVersion { .. }) => {}
    ...
    async fn test_kv_snapshot_lease_lifecycle() {
        ...
        let lease = KvSnapshotLeaseForTable {
            table_id,
            bucket_snapshots: vec![KvSnapshotLeaseForBucket {
                partition_id: None,
                bucket_id: 0,
                snapshot_id: 0,
            }],
    ...
    async fn test_producer_offsets_lifecycle() {
        ...
        let table_offsets = vec![ProducerTableOffsets {
            table_id,
            bucket_offsets: vec![BucketOffset {
                partition_id: None,
                bucket_id: 0,
        ...
        admin
            .register_producer_offsets(producer_id, table_offsets, None)
            .await
    ...
    async fn test_acl_lifecycle() {
        ...
        match admin.create_acls(vec![acl.clone()]).await {
            ...
            Ok(_) => {
                ...
                assert!(
                    listed.iter().any(|a| a.resource_name == db_name),
