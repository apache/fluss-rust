...
impl UpsertWriterFactory {
    pub fn create(
        table_path: &Arc<TablePath>,
        table_info: &TableInfo,
        partial_update_columns: &Arc<[usize]>,
        writer_client: &Arc<WriterClient>,
        ...
        Ok(UpsertWriterImpl {
            ...
            primary_key_encoder,
            target_columns: Arc::clone(partial_update_columns),
            bucket_key_encoder,
    ...
    fn sanity_check(
        ...
        let mut target_column_set = BitSet::with_capacity(field_count);

        for &target_index in &target_columns {
            target_column_set.insert(target_index);
        }

        let mut pk_column_set = BitSet::with_capacity(field_count);
        ...
        let mut auto_increment_column_set = BitSet::with_capacity(field_count);
        ...
        for auto_increment_col_name in &auto_increment_col_names {
            ...
            if let Some(index) = auto_increment_field_index {
                if target_column_set.contains(index) {
                    return Err(IllegalArgument {
                        message: format!(
                ...
                auto_increment_column_set.insert(index);
            }
        }
        ...
        for i in 0..field_count {
            ...
            if !pk_column_set.contains(i) && !auto_increment_column_set.contains(i) {
...
impl BitSet {
    ...
    fn insert(&mut self, value: usize) {
        let w = Self::word_index(value);
        self.bits[w] |= Self::mask(value);
    }
