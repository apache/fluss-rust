...
The Rust client is not yet published to crates.io. You need to build from source.
...
Fluss-rust currently has to be included locally. In your project, point to fluss-rust crate:

```toml
[dependencies]
fluss = { path = "/path/to/fluss-rust/crates/fluss" }
...
fluss = { git = "https://github.com/apache/fluss-rust.git" }

# With S3 storage support
fluss = { git = "https://github.com/apache/fluss-rust.git", features = ["storage-s3"] }

# With OSS storage support
fluss = { git = "https://github.com/apache/fluss-rust.git", features = ["storage-oss"] }

# All storage backends
fluss = { git = "https://github.com/apache/fluss-rust.git", features = ["storage-all"] }
```
...
append_writer.append(&row2).await?;
...
// Subscribe from a specific timestamp
log_scanner.subscribe_by_timestamp(0, timestamp_ms).await?;
